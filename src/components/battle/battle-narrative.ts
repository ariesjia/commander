...
export function randomPlayerDodgeLine(enemyAttack: string): string {
...
export function randomEnemyDodgeLine(ourAttack: string): string {
  const ours = ourAttack.trim() || randomPick(PLAYER_ACTIONS);
  return randomPick([
    `【敌方】${ours}被闪掉了，对方溜得很快！`,
...
export function randomFinishLoseLine(enemyAttack: string): string {
  const a = enemyAttack;
  return randomPick([
    `【敌方】${a}使出致命一击，我们遭到重创！`,
    `【敌方】${a}致命一击落下，我们遭到重创！`,
